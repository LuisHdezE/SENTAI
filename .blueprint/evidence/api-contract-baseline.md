...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| API-AUTH-001 | Identity & Access | BO, CP, MOB | Todos | UC-001 | FR-022 | N/A | Login / Establecer sesión | POST | `/api/v1/auth/login` | Command | pre-auth (establishes session) | N/A | N/A | NOT_REQUIRED | `auth.login.success` | not required | Sesión/Token emitido | authentication | TBD: Estructura exacta payload offline. |
...
| API-INV-005 | Inventory | BO | ACT-003 | UC-006 | FR-005 | N/A | Mover Inventario | POST | `/api/v1/inventory/move` | Command | required | `inventory.adjust` | N/A | REQUIRED | `inventory.adjustment.applied` | required | Stock reubicado lógicamente | domain_conflict | |
| API-INV-006 | Inventory | BO | ACT-003 | UC-006 | FR-005 | BR-004 | Bloquear Ubicación | POST | `TBD` | Command | required | `inventory.location.block` | N/A | REQUIRED | `inventory.adjustment.applied` | required | Ubicación bloqueada | domain_conflict | TBD: Path de bloqueo exacto no definido. |
